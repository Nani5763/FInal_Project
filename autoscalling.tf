#Create Autoscalling Group Resource for Frontend
...
#Autoscalling Group Resource for Backend
resource "aws_autoscaling_group" "backend-asg" {
    name_prefix = "backend-asg"
    desired_capacity = 1
    max_size = 1
    min_size = 1
    vpc_zone_identifier = [ aws_subnet.private-subnet-5.id, aws_subnet.private-subnet-6.id ]
